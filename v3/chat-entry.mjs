...
function renderChat(){
 ...
 el('chat-send').disabled=busy||chatCustomBusy||!state;
 ...
 if(s?.worker?.running){chatAnswer('<p>正在检查。完成后会告诉你先改哪里。</p><a href="/workbench#execution">查看进度或暂停</a>');return;}
 if(ready){
  const failed=(state.automatic?.decisions||[]).find(d=>d.outcome==='requirement-not-met');
  chatAnswer('<p>'+(issues.length?'先处理这 '+issues.length+' 项偏差。'+(failed?esc(failed.summary):'打开证据，按位置修复问题。'):'查看本轮结果，再核对尚未覆盖的目标。')+'</p><p>修改后，告诉我“重新检查”，我会按原目标再跑一轮。</p><a href="/report">查看问题与证据</a><a href="/codex-context.md" download>把修改要求带回 Codex</a>');
  ...
  chatAnswer('<p>接下来检查“'+esc(t.goal)+'”。'+(reminders.length?'会一并核对：'+esc(reminders[0].summary):'')+'</p><details><summary>查看本次检查范围</summary><p>'+esc(t.limitations)+'</p><ul>'+t.checks.filter(c=>!t.excludedPaths.includes(c.id)).map(c=>'<li>'+esc(c.module)+'：'+esc(c.expected)+'</li>').join('')+'</ul></details><button type="button" id="chat-start">开始检查</button>');
...
if(el('chat-home')){
 ...
 el('chat-form').onsubmit=async e=>{
  ...
  if(/^(怎么做|怎么用|帮助|使用指南)[？?。！!]*$/.test(text)){
   chatAnswer('<p>先写目标，例如：帮我检查报告，详情应该默认收起。</p><p>生成计划后点“开始检查”；修复问题后，输入“重新检查”。</p><p>想先体验，输入“查看案例”。换项目输入“设置项目”；留意见用“纠正：具体要求”。</p><a href="/guide-video">看 3 分半使用演示</a>');return;
  }
  if(/^(设置项目|项目设置|换项目)[。！!]*$/.test(text)){
   chatAnswer('<p>先填入本地项目目录、页面地址和相关资料，再回到这里说目标。</p><a href="/workbench#project-settings">设置项目与资料</a>');return;
  }
  ...
  if(!el('project-path').value||!el('url').value){chatAnswer('<p>先告诉我需要检查哪个项目。</p><a href="/workbench#project-settings">填入项目目录与页面地址</a>');return;}
