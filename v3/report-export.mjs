...
export function pdfSummaryHTML({task,result,automatic,impacts,coverage,helpEvaluation,repair=null}){
 ...
 return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>body{font:12px/1.65 system-ui,"PingFang SC",sans-serif;color:#172e46}h1{font-size:26px}h2{font-size:19px;border-bottom:1px solid #bdc8d2;padding-bottom:8px;margin-top:28px}h3{font-size:14px}article{padding:8px 0;border-bottom:1px solid #dbe2e8}p{margin:5px 0;overflow-wrap:anywhere}blockquote{margin:8px 0;padding:8px 12px;background:#f4f6f8}.note{color:#4e6072;font-size:10px}.check{padding-left:12px}.attempt{margin:10px 0}h2,h3{break-after:avoid}.attempt{break-inside:avoid}</style></head><body><h1>本轮验收报告</h1><p>目标：'+e(task.goal)+'</p><p>检查范围：'+e(task.endpoint)+'</p><p class="note">'+e(result.generatedAt)+' · 各项状态分别报告；入口通过不等于整个产品通过。</p><h2>需要关注的问题</h2>'+(issueHTML||'<p>已覆盖路径没有确认偏差；未验证部分见下文。</p>')+repairHTML(repair,{download:false})+helpEvaluationHTML(helpEvaluation)+'<h2>个人经验实际改变了什么</h2>'+(help||'<p>本轮仅使用当前目标与通用检查。</p>')+'<p class="note">落实要求不等于已证明长期减少返工。</p><h2>未验证与覆盖边界</h2><ul>'+gaps.map(x=>'<li>'+e(x)+'</li>').join('')+'</ul><h2>复检与证据索引</h2><p class="note">PDF 保留检查结果与文件索引，不嵌入原始截图或证据附件。完整证据须与本轮网页报告或运行目录一起保存。</p>'+records+'</body></html>';
}
